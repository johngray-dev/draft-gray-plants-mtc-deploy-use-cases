...
struct {
    ...
    HashValue subtree_root;

    HashValue previous_landmark_hash;

    HashValue landmark_transition_path<0..2^16-1>;
...
start:
   The first leaf index covered by the subtree authenticated by
   this landmark.

end:
   The last leaf index covered by the subtree authenticated by
   this landmark.

subtree_root:
   The Merkle Tree root calculated for the subtree identified by
   start and end.

previous_landmark_hash:
   The hash of the previously published landmark.  For the first
   landmark in a series, this value SHALL be omitted or set to
   the all-zero hash value.

landmark_transition_path:
   A sequence of hashes required to authenticate the previous
   landmark within the current landmark.  The exact calculation
   of this value is implementation dependent but SHALL permit a
   verifier to construct a verifiable chain between successive
   landmarks.

signatures:
   One or more signatures generated by the issuer over the
   contents of the landmark.


When a new landmark is published, the issuer calculates the
landmark_transition_path that links the previous landmark to the
current landmark.  This computation is performed once at
publication time and does not require subsequent recalculation.

A verifier that possesses a landmark for the subtree identified
by the start and end values from the MTCProof structure may
retrieve newer landmarks from the Landmark Distribution Point
until reaching a locally trusted landmark.

For each successive landmark, the verifier SHALL verify the
issuer signature and SHALL verify the landmark_transition_path
from the previously validated landmark.  Successful validation
establishes an authenticated chain from the subtree referenced
by the certificate inclusion proof to the trusted landmark.

This mechanism allows incremental retrieval of validation data
while avoiding the need to redistribute complete historical
Merkle Tree state.
